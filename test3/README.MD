...
```
sql代码：
ALTER USER luzheng QUOTA UNLIMITED ON USERS;
ALTER USER luzheng QUOTA UNLIMITED ON USERS02;
ALTER USER luzheng QUOTA UNLIMITED ON USERS03;
```
  ...
  分析：首先需要用限额无限语句'QUOTA UNLIMITED',意味着将新建用户luzheng在USERS、USERS02、USER03三个表空间中的配额权限修改为无配额限制。(在这里要注意，点击左上角第二个按钮最好，即执行文本。如果后面有USER02,USER03无权限，可能就是这里的问题)
